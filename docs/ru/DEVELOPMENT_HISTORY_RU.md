...
- **Система компоновки главной страницы (HomePageBlock)**: Реализация универсальной системы управления
  блоками контента на главной странице с двумя режимами отображения.
  ...
  - **Режимы отображения**:
    ...
    - Переключение режима доступно пользователю в настройках темы (рядом с dark/light mode).
  ...
  - **Параметры блока** (универсальные для всех блоков):
    ...
    - **Термины таксономии**: Выбор от 1 до всех терминов для фильтрации новостей в блоке.
    - **Количество новостей**: Общее число выводимых новостей в блоке.
    - **Отображение контента**:
      - Заголовок (обязательно)
      - Тизер (опционально, по галочке)
    - **Размер шрифта заголовка**: Относительно базового размера (меньше базового).
  
  - **Ограничения**: Максимум 50 блоков на главной странице.
  
  - **Компоненты Backend**:
    - Сущность `HomePageBlock` с универсальными параметрами
    ...
    - Admin API: CRUD операции для управления блоками
  
  - **Компоненты Frontend**:
    - UI переключения режима в настройках пользователя
    - Компонент рендеринга блоков для Custom режима
    - Компонент простого списка для Simple режима
    - Admin UI для создания и настройки блоков

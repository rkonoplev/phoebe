...
- **Система компоновки главной страницы (HomePageBlock)**: Реализация гибкой системы управления блоками
  контента на главной странице.
  - **Назначение**: Предоставить возможность структурировать главную страницу через блоки с привязкой к
    терминам таксономии, настройкой порядка отображения (вес), количества новостей и параметров
    отображения (заголовок/тизер).
  - **Компоненты**: Новая сущность `HomePageBlock`, Public API эндпоинт `/api/public/homepage`, Admin API
    для управления блоками, UI в админ-панели для создания и настройки блоков.
  - **Оценка**: 4-6 дней (Backend: 2-3 дня, Frontend: 1-2 дня, Тестирование: 1 день).

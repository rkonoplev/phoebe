...
### Текущая схема базы данных `phoebe_db` (После всех миграций V1-V14)
...
```bash
# Подключение к MySQL с пустым паролем (если контейнер запущен с MYSQL_ROOT_PASSWORD=)
docker exec -it temp_mysql_explorer mysql -uroot

# Подключение к MySQL с известным паролем (если контейнер запущен с MYSQL_ROOT_PASSWORD=<пароль>)
# В этом случае, после выполнения команды, MySQL запросит пароль.
# docker exec -it temp_mysql_explorer mysql -uroot -p

# Альтернативный способ найти контейнер, если вы не знаете его имя или ID
# (может выбрать не тот контейнер, если их несколько)
# docker exec -it $(docker ps -q --filter ancestor=mysql:5.7) mysql -uroot -p
```

**Пояснение к подключению:**
*   **`docker exec -it <ИМЯ_КОНТЕЙНЕРА> mysql -uroot`**: Это предпочтительный способ, если вы запустили контейнер с пустым паролем для `root` (как в Шаге 1). Флаг `-p` отсутствует, и MySQL попытается подключиться без пароля.
*   **`docker exec -it <ИМЯ_КОНТЕЙНЕРА> mysql -uroot -p`**: Используйте этот вариант, если вы знаете, что у `root` есть пароль. MySQL запросит его после выполнения команды.
*   **`$(docker ps -q --filter ancestor=mysql:5.7)`**: Эта конструкция динамически находит ID запущенного контейнера, использующего образ `mysql:5.7`. **Будьте осторожны:** если у вас запущено несколько таких контейнеров, эта команда может выбрать любой из них. Рекомендуется использовать конкретное имя контейнера, если оно известно.
...
1.  **Дамп первой базы данных:**
    ...
    *   Если у `root` есть пароль, добавьте `-p<ПАРОЛЬ>` (например, `-proot`) после `-uroot`.
...
### Шаг 4: Остановка и удаление временного контейнера

После того как вы получили все необходимые дампы или закончили исследование:

1.  **Остановите контейнер:**
    *   Если вы запустили контейнер с флагом `--rm` (как в Шаге 1), то достаточно просто **закрыть окно терминала**, в котором был запущен `docker run ...`. Контейнер автоматически остановится и удалится.
    *   Если вы запустили контейнер без `--rm` или хотите остановить его из другого терминала, используйте:
        ```bash
        docker stop temp_mysql_explorer
        ```
        (Замените `temp_mysql_explorer` на актуальное имя или ID вашего контейнера).

2.  **Удалите контейнер (если он был запущен без `--rm`):**
    ```bash
...
### Автоматические миграции (Flyway, версия 11.19.1)
...
| V12 | Блоки главной страницы | Добавлены таблицы `homepage_block` и `homepage_block_item` для динамических блоков контента | `common` |
| V13 | Сделать временные метки контента непустыми | Колонки created_at/updated_at в таблице content стали NOT NULL. | `common` |
| V14 | Сделать временные метки других таблиц непустыми | created_at/updated_at в других таблицах стали NOT NULL. Переименована из V12. | `common` |

...
-   **Make**: (Рекомендуется) Для выполнения команд из `Makefile`.
    -   **macOS**:
        -   **Способ 1 (Рекомендуемый)**: Установите Xcode Command Line Tools, выполнив команду `xcode-select --install`. Это легковесная установка, которая не требует полной версии Xcode.
        -   **Способ 2 (Homebrew)**: Если вы используете Homebrew, вы можете установить `make` с помощью команды `brew install make`.
    -   **Linux (Debian/Ubuntu)**: Установите с помощью `sudo apt-get install build-essential`.
    -   **Windows**: Можно установить с помощью Chocolatey (`choco install make`) или используя Windows Subsystem for Linux (WSL).

...
| `make run-no-cache` | **Пересобрать образы без кэша**. | Исправление проблем с "застрявшим" кэшем Docker. |
...
Если приложение ведет себя неожиданно после изменений в коде (особенно в `build.gradle` или файлах миграций), Docker может использовать устаревшую, кэшированную версию файлов.

1.  **Полный сброс окружения (удаляет данные в БД):**
    ```bash
    make reset
    ```
...
2.  **Принудительная пересборка без кэша:**
    ...
    make run-no-cache
    ```
    Эта команда заставит Docker пересобрать образы с нуля, игнорируя кэш. Сборка будет дольше, но это решает проблемы с "застрявшими" файлами.
